...
* During the installation, ensure that you check the box for installing `pip` alongside Python 3.10+.
* Ensure Python has been added to your path by opening a new terminal and entering `python --version`. This should report your Python version as 3.10 or higher.
* Run the following command in your terminal: `python -m pip install --upgrade pip`
* Verify `pip` install by typing: `python -m pip --version`
...
* Install CUDA and cuDNN versions that match your TensorFlow 2.x build.
* Follow the official TensorFlow install guide or use WSL2/Linux for the smoothest GPU setup on Windows.
* After installation, ensure CUDA and cuDNN are on your system PATH, then restart your PC.

### Install TensorFlow 2.x

* Open a new terminal and run `python -m pip install --upgrade tensorflow`
* Test installation: `python -c "import tensorflow as tf;print(tf.reduce_sum(tf.random.normal([1000, 1000])))"`
* Note: On Windows, native GPU support depends on your TensorFlow build. For GPU acceleration, consider using WSL2 or a CUDA-enabled Linux environment.

### Setting up this Repository

* `git clone https://github.com/Raghav-B/poke.AI` to your desired directory.
* `cd object_detection/keras-retinanet`
* `python -m pip install .` - This should install all remaining dependencies.
